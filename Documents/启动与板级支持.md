...
> 解耦目标：加板 / 换固件 / 加驱动时 **尽量不改 Common**。排期见 [`路线图.md`](路线图.md) **1.3b**（板包）与 **1.3d**（Drv）；分层见 [`架构分层.md`](架构分层.md)。
...
| **Drv 设备类** | Probe→Match→Bind→ops（Block/Input/Net/Display/Audio…） | 驱动 `.c` + 一行注册；见 [`驱动框架.md`](驱动框架.md) |
...
- 加声卡等 ≈ Drv 新类或范例驱动 + 注册，不改 Gui/FAT。  
...
- **加驱动**：实现类 ops + `ToyDrvRegister`；声卡等可增 **Audio** 类（先 HAL 私有 tone 也可，再抬门面）。  
...
| `Board.c` / `Board.h` | B2/B3 | Startup 填充辅助、兼容表；**不**进 Common |
| `NOTES.md` | 可选 | 真机踩坑 |
...
**Makefile `BOARD=`**：见路线图 **B2**（本刀只定包形状）。
...
| 文档概念 | 路线图 |
|----------|--------|
| Board 包约定、不改 Common、厂商 U-Boot | **1.3b**：**B0 ✅**（清单/模板）；B1～B3（Duo S = UART 命令行靶） |
| Arch vs Board 分离、能力旗标 | **B1**（`HalHasFrameBuffer` / `HalConsoleOnly`） |
| Drv / 声卡等设备类 | **1.3d ✅**（D1～D4；Audio 后置可记 1.4） |
| x86 桌面真机（UEFI GOP） | **1.3c**（与 1.3b **互不混仓**） |
