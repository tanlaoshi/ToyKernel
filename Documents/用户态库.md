...
PR 顺序建议：**L1** ✅ → **L2** ✅ → **L3** ✅（GUI ABI/入门）→ **L4**。
...
| `User/Library/ToyOs/libtoyos.a` | string/printf/malloc/errno/unistd/stdlib/signal |
...
破坏性变更升 MAJOR；像素 blit 等新能力升 MINOR 或另开 API。细节与教程见 [`用户态GUI入门.md`](用户态GUI入门.md)。

---

## 5. 成功标准（库侧）

- 学生能：`#include <stdio.h>` / `<ToyUi.h>` 写出程序并 `exec`。
- 模板：`User/pkg` / `User/pkg/gui` 一条 `make` 出 `*.ELF`（**L1/L3** ✅）。
- libc 扩展：`atoi`/`qsort`/`snprintf`/`signal`（**L2** ✅）；dir/`stat` 见 F4。
- 文档：「如何写第一个 GUI 程序」（**L3** ✅ [`用户态GUI入门.md`](用户态GUI入门.md)）。
