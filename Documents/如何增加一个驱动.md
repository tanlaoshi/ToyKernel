...
|--------------|--------|---------------|--------------|
| 硬盘 / SSD | **Block** | `BlockRead/Write*`、VFS/FAT | `ata-pio`、`ahci`（H1）、`virtio-blk` |
| 键盘 / 鼠标 | **Input** | `HalKeyboard*` / `HalMouse*` | `xhci-hid`、`virtio-input` |
...
| virtio-blk | ✅ | virt |
| **NVMe** | **规划提上 → H5** | PCIe NVMe；仍只挂 Block 类；Common FAT/VFS **不动** |
| USB MSC | 后置 | 与安装器分开 |

加 NVMe 的期望工作量：PCI 找 class `08h` subclass `02h` → 管理队列/IO 队列最小集 → `ToyDriverBlockAttach`。**不要**改 VFS。
