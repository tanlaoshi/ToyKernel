...
| 层 | 目录 / 仓库 | 职责一句话 |
|----|-------------|------------|
| **Boot** | 各 `HAL/<Arch>/Startup`；x86 另有 `ToyBoot/` | 填 `BOOT_INFO` 交给 Core。**x86** = UEFI/ToyBoot；**Arm/RiscV** = 自有 Boot（virt + DTB / OpenSBI `-kernel`） |
| **HAL** | `Include/Hal*.h` + `HAL/<Arch>/` | **全部**架构相关代码在此；Common **只**经门面访问硬件 |
| **Fonts** | `Common/Fonts/` + `Include/Font.h` | 点阵字体注册与度量；绘制层不 include 某份字模表 |
| **Library** | `Common/Library/` | 可复用算法/协议（块设备、FAT、ELF…），无调度语义 |
| **Core** | `Core/` | 启动组装、内存、调度、进程、系统调用 |
...
QEMU 布局：`run-split.sh` 盘0=ESP（仅 EFI），盘1=`RootFs/X64/`（Kernel/THEME/用户 ELF）。
...
| 定时器 | `HalTimerInitialize`/`Start`/`Ack`/`SetInterval` |
...
| 头文件 | 对外 API 摘要 |
|--------|----------------|
| `HalSerial.h` | `HalSerialInitialize`、写字节/字符串、`HalSerialDataReady` |
...
| API（`Font.h`） | 说明 |
|-----------------|------|
| `FontInitialize` / `FontCount` / `FontCurrentId` | 注册表 |
| `FontGetById` / `FontGetCurrent` / `FontSetById` | 切换字体 |
| `FontCellW/H`、`FontAdvanceX/Y`、`FontGlyph` | 度量与字形指针 |

**加字体**：`Common/Fonts/` 增加数据 + `FontRegistry` 注册一行；Gui/Video **不**直接绑死某 `.c`。
...
| CString | （内部） | 简易字符串 | — |
| LibWrite（**R4**） | `LibWrite.h` | ListDir 等文本槽；ConsoleInitialize 注册 `ConsoleWrite`；未注册 → `HalDebugWrite` | Console |
...
| PMM | `PhysicalMemory.h` | `Allocate/Free(Pages)`、`Retain/Release`（COW）；256 段 × 1GB，只交出恒等窗口内的页（x86 512MB，arm64/riscv 4GB） |
...
| 系统调用 | `Syscall.h` | `SyscallInit`、`SyscallDispatch`；号 0–26（含 `brk`/`mmap`）见下 |
...
| 2 | `open` | 路径 → fd（**流式**：记路径/`Size`/`Pos`；非整文件缓冲） |
| 3 | `read` | 按 `Pos` 分块读盘（上限见文件长度） |
| 4 | `close` | 目录快照释放；文件已写穿盘 |
| 5 | `fork` | COW |
| 6 | `wait` | `rdi=WNOHANG` 可选 |
| 7 | `yield` | |
| 8–12 | `socket`…`accept` | 默认内核已编入 lwIP；见 `Socket.h` |
...
| 25 | `mmap` | 匿名；`rdi=len rsi=prot rdx=flags` → VA（**PR-U-mmap**） |
| 26 | `munmap` | `rdi=addr rsi=len` → 0 |
| 27 | `signal` | 注册 handler |
| 28 | `damage_rect` | 用户窗像素矩形 |
| 29 | `lseek` | `rdi=fd rsi=off rdx=whence` → 新 Pos；管道/套接字失败（**PR-A-libc**） |
| 30 | `sleep` | `rdi=ms`；阻塞约 Ms 个调度节拍（课堂 ≈ms；上限 60000） |
| 31 | `clock_ms` | 返回 `HalCpuTicks(0)`（与 sleep 同尺，稳节拍） |
...
| Udp / Tcp | `Udp.h`/`Tcp.h` | 内核态自研栈（**教学对照**） | Shell builtin（未 `lwip on`） |
| LwIp | `LwIp.h` | 默认生产栈开关/状态 | Shell；`LWIP=0` 可关 |
...
| libToyNet（**L4** ✅） | `#include <ToyNet.h>`；默认内核已 `LWIP=1` |
...
| `ToyImage/` | QEMU 镜像：`run-split.sh`（ESP + `RootFs/X64/`；`run.sh` 已转发） |
...
```
Core/VirtualMemory.c   地址空间、Clone、CopyFrom/ToUser
        ↓ HalPage*
...
| Core | `Include/BootInfo.h` + `Core/BootInfo.c` | `BOOT_INFO` / 内存区域；`BootInfoGet()` 只读 |
...
| 9 | `usb` | `InitializeUsb` | xHCI HID（+ `ps2-kbd` mux）；**EHCI** Probe/CCS（`ehci`，PR-H-ehci-1；HID→ehci-2） |
...
| 文件 | 说明 |
|------|------|
| `Core/Kernel.c` | `KernelMain`、创建任务、`exec`/`ps`/网络命令 |
| `Core/KernelModules.c` | `gModules[]` 与各 `Init*`（启动顺序见第 2 节） |
| `Include/BootInfo.h` + `Core/BootInfo.c` | 架构无关启动信息 |
...
| `PhysicalMemory.c` / `PhysicalMemorySeg.c` / `PhysicalMemoryAlloc.c` | 分段位图 PMM：256 段 × 1GB；段内 UINT16 refcount；总量/空闲为各段之和 |
...
| `UI.c/h` | 几何与控件绘制 |
| `Gui*.c` / `Gui.h` / `GuiPrivate.h` | 窗口（Wm）、合成、拖动、光标、焦点、用户窗；对外仍 `Gui.h` |
| `FontData.h` | 字形 |
...
| `ToyImage/run-split.sh` | 盘0 Boot，盘1 `RootFs/X64/`；`--kill-qemu` / `--headless` / `TOY_SMP` |
| `ToyImage/smoke-boot.sh` | 无头冒烟等到 `ToyOS ready`（PR-Q1） |
| `ToyImage/QUICK_START.md` | 双盘布局与 SMP 排查；真机指针 |
| [`Documents/已完/真机冒烟清单.md`](已完/真机冒烟清单.md) | **PR-PC-smoke**：上电→Boot→桌面；键/盘/网勾选 |
...
- PMM 跟踪上限 256GB（256 段 × 1GB）；分配只返回恒等窗口内的页（x86 512MB，arm64/riscv 4GB）；`gPhysLock` 保护分配与引用计数；任务栈 4KB 嵌在 `TASK` 内
- TCP 无重传；GUI 无完整控件集
...
| FileSystem.c | FileSystemInitialize, MountAllVolumes, Fs*, vols |
...
| A4 | `Core/Kernel.c` 的 `KernelMain` + `KernelModules.c` | 模块表（第 2 节）、创建任务 |
| A5 | `Core/Module.c` | 顺序 Init 框架 |
...
| 顺序 | 文件 | 说明 |
|------|------|------|
| C1 | `PhysicalMemory.c` | 读 `BootInfoGet()` 区域，按 1GB 段建位图并分配 |
...
| 顺序 | 内容 |
|------|------|
| I1 | 保持 FAT + Block 接口不变，实现 NVMe 或 AHCI 后端 → **H1/H5 ✅** |
| I2 | ToyBoot：ESP 写入 / 可选建分区（慎改 GPT）→ **PR-H-msc** |
| I3 | GRUB `chainloader` 文档（见 `ToyBoot/README.md`） |
| I4 | **PR-PC-smoke ✅**：[`真机冒烟清单.md`](已完/真机冒烟清单.md)（上电→Boot→桌面；键/盘/网） |
...
| 冒烟勾选 | 板 README | [`真机冒烟清单.md`](已完/真机冒烟清单.md)（**PC-smoke**） |
...
> **新人短页**：[`驱动/驱动开发指南.md`](驱动/驱动开发指南.md)（5 步 + Demo）；拷贝源：[`../HAL/X64/Drivers/_template/`](../HAL/X64/Drivers/_template/)。  
> 匹配：[`驱动/驱动匹配规范.md`](驱动/驱动匹配规范.md)；设计稿已归档 [`已完/驱动模板设计.md`](已完/驱动模板设计.md)。  
...
| 键盘 / 鼠标 | **Input** | `HalKeyboard*` / `HalMouse*` | `xhci-hid`、`ps2-kbd`（H2）、`ehci`（CCS；HID→ehci-2）、`virtio-input` |
...
现状：课堂/QEMU 主要是 **virtio-net**；真机常见 **Intel e1000/e1000e、Realtek RTL81xx、其它 PCI NIC**。栈已在 Common（**默认 lwIP**；builtin = 教学对照），缺的是 **HAL Net 后端**（e1000e 见 H4e）。
...
**真机网络能否用？**  
- 协议栈：能（同一套 `HalNet*` / 默认 lwIP）。  
- 驱动：课堂默认 virtio；QEMU **e1000/e1000e（H4/H4e）**；NUC **I219（`156F`）课路径 ✅** → [`已完/I219真机网课路径.md`](已完/I219真机网课路径.md)。  
- 无线 / Realtek / 复杂 offload：**原「明确不做」已改**——有线 alx/r8169 与无线 MVP 见 [`驱动/网卡扩展-高通Realtek无线.md`](驱动/网卡扩展-高通Realtek无线.md) 与路线图 `PR-N-alx-*` / `PR-N-rtl-*` / `PR-N-wifi-*`；**仍不做**全芯片族产品化与重 offload。
...
| **DTB 兼容串**（后续板包） | 匹配表字段 `TOY_DRIVER.Match` 预留；当前范例多为 `Match = 0`，在 Probe 内扫。规范：[`驱动/驱动匹配规范.md`](驱动/驱动匹配规范.md) |
...
static const TOY_DRIVER gMyDriver = {
    ...
    .Match = 0,
    .Probe = MyProbe,   /* 有设备 → 0，并可选 *OutPrivate；无 → 非 0 */
    .Bind  = MyBind,    /* 挂上类后端 */
...
cd ../ToyImage && ./smoke-virt.sh   # 或 ./run-virt-arm.sh --headless
...
| 库 / 模块 | 内容 | 备注 |
|-----------|------|------|
| **libc 子集** | `string` / `stdio`（含 `fopen`/`snprintf`）/ `stdlib`（`atoi`/`qsort`/`realloc`）/ `errno` / `unistd`（含 `lseek`） | ✅ **L1+L2+A-libc**；非完整 POSIX |
| **libtoyos** | 稳定 syscall 包装 + 版本宏；`#include <toyos/syscall.h>` / `<toyos/version.h>`；静库 `User/Library/ToyOs/libtoyos.a` | ✅ **L1/L2/A-libc** + **sleep**（CRT **1.4.0**） |
...
| 库 | 内容 | 依赖 |
|----|------|------|
| **libToyGfx** | 点/线/矩形/blit（位图仍缺）；ABI **1.2.0** | CRT；✅ G15 + **L3** + **A-gfx-api** |
| **libToyUi** | 按钮/标签/复选框/列表/输入框 + 键盘入窗；ABI **1.2.0** | libToyGfx；✅ G15 + **L3** + **A-ui-api** + 键盘入窗 |
| **libFsUtil** | 路径、目录遍历；多卷 `TOYOS:` 前缀；ABI **1.0.0** | CRT；✅ **A-fsutil** |
...
| 库 | 内容 |
|----|------|
| **libnet** / **libToyNet** | BSD socket 薄封装；ABI **1.2.0**；`#include <ToyNet.h>` | ✅ **L4** + **A-net-dns**；默认内核 `LWIP=1` |
...
|----------------------|--------------------------------|------|
| `write` / `read` / `open` / `close` | ✅ FD；文件按偏移流式（**U-stream-1**；写上限 `FAT_WRITE_MAX` 8MiB） | CRT / stdio |
...
| `socket` / `bind` / `connect`… | ✅（默认 `LWIP=1`） | **libToyNet**（L4） |
...
| 库 | 版本宏 | 公开符号 |
|----|--------|----------|
| libToyGfx | `TOY_GFX_ABI_VERSION_*` **1.2.0** | `ToyGfxDamageText` / `DamageRect` / `DrawPixel` / `DrawLine` / `FillRect` / `DrawRect` |
| libToyUi | `TOY_UI_ABI_VERSION_*` **1.2.0** | `CreateWindow` / `SetLabel` / `AddButton` / `Poll`（含 `300+HID`）+ 复选框/列表/输入框 |
| libToyNet | `TOY_NET_ABI_VERSION_*` **1.2.0** | `socket`/`connect`/`bind`/`listen`/`accept`/`send`/`recv` + `ToySockAddrIn` / `ToyNetResolve` |
| libFsUtil | `FS_UTIL_ABI_VERSION_*` **1.0.0** | `FsUtilJoin` / `VolumePath` / `ToyosPath` / `ListDir` / `Stat` |
| CRT / libtoyos | `TOYOS_CRT_VERSION_*` **1.4.0** | + `sleep`/`usleep`/`msleep`/`clock_ms`（**sleep 稳节拍**） |

破坏性变更升 MAJOR；像素 blit 等新能力升 MINOR 或另开 API。GUI 教程见 [`用户态GUI入门.md`](#iv-用户态与桌面资产--第一个-gui-程序)。  
网络：`User/Pkg/Net/` + `NETLIB.ELF`；默认路径 ping → `dns` → `NETLIB`（见 [`ThirdParty/README.md`](../ThirdParty/README.md)）。
...
- libc 扩展：`atoi`/`qsort`/`snprintf`/`signal`（**L2** ✅）；`OpenDirectory`/`ReadDirectory`/`FileStat`（**F4** ✅）；`lseek`/`fopen`/`realloc`/`strstr`（**A-libc** ✅）。
...
|-----------|-------------|------------|------|------|
| **CRT / libtoyos** | `User/crt/*` → `User/Library/ToyOs/libtoyos.a` | `TOYOS_CRT` **1.4.0** | ✅ | L1/L2 + F4 + A-libc + **sleep** |
| **libToyGfx** | `User/Library/ToyGfx/` → `libToyGfx.a` | **1.2.0** | ✅ | 文字 + blit + 点/线/矩形；无位图 |
| **libToyUi** | `User/Library/ToyUi/` → `libToyUi.a` | **1.2.0** | ✅ | 窗/按钮 + 复选框/列表/输入框 + 键盘 |
| **libToyNet** | `User/Library/ToyNet/` → `libToyNet.a` | **1.2.0** | ✅ | sockaddr 封装 + DNS；Guest `lwip on` |
| **libFsUtil** | `User/Library/FsUtil/` → `libFsUtil.a` | **1.0.0** | ✅ | 路径拼接 / 列目录 / `TOYOS:`；底层 F4 |
...
| `<stdlib.h>` | `malloc`/`free`/`exit`/`atoi`/`atol`/`abs`/`qsort`；**realloc / calloc** | 无 `strtol`/`getenv` |
| `<string.h>` | `strlen`/`strcmp`/`strncmp`/`strcpy`/`strncpy`/`mem*`；**strstr / strchr / strcat / strdup** | — |
| `<unistd.h>` | `read`/`write`/`close`/`lseek`/`fork`/`wait`/`pipe`/`dup`/`execve`/`brk`/`kill`/`mmap`/`munmap` + 窗口；**`sleep`/`usleep`/`msleep`/`clock_ms`** | 无 `getcwd`/`chdir`/`getpid`；mmap 仅匿名；sleep≈调度节拍非墙钟 |
| `<sys/mman.h>` | `mmap`/`munmap`；`PROT_*`/`MAP_ANONYMOUS` | 无文件映射 / `MAP_FIXED` / `mprotect` |
...
| `<toyos/syscall.h>` | SYS 0～31 + `toy_*` 包装 | 改号须同步内核 |

### 6.3 libToy* 能力 vs 规划文案

| 规划期望（§B/C） | 现状 | 对 V2 / 后置的含义 |
|------------------|------|-------------------|
| Gfx：点/线/矩形/blit/位图 | 点/线/矩形/blit ✅ **1.2.0**；位图仍缺 | 简易编辑器可不依赖位图 |
| Ui：按钮/标签/事件 | 按钮 + 复选框/列表/输入框 + **键盘入窗**（`300+HID`）✅ | 非完整 toolkit；无滚动条/菜单 |
| Net：BSD socket | `connect(fd,ip,port)` + `ToySockAddrIn`；`ToyNetResolve` 域名 ✅ **A-net-dns** | 完整 POSIX `getaddrinfo` / 网络序 sockaddr 后置 |
| FsUtil：路径、多卷 | `FsUtilJoin` / `ListDir` / `TOYOS:` ✅ **A-fsutil** | 仍可直调 dirent；无 getcwd |
...
| ELF / 源 | 覆盖 |
|----------|------|
| `HELLO` / `LIBCDEMO` / `SLEEPDEMO` / `BRKDEMO` / `FORK` / `PIPEDEMO` / `EXECDEMO` / `KILLDEMO` | CRT / 进程 / sleep |
...
3. **值得后加的小刀**（非本柱必做）：`getcwd`/`sleep`/`getpid`、flags 真生效、libm。
...
cp MYAPP.ELF ../ToyImage/RootFs/X64/
...
cp build/MYGUI.ELF ../../../ToyImage/RootFs/X64/MYGUI.ELF
# QEMU（run-split.sh）Shell 中：
#   toyos> exec MYGUI.ELF
```

或直接跑仓库自带演示：`exec GUIDEMO.ELF`（源码 `User/Apps/GuiDemo.c`）。
...
| libToyGfx | `TOY_GFX_ABI_VERSION_*` | **1.2.0** |
| libToyNet | `TOY_NET_ABI_VERSION_*` | **1.2.0** |
...
| `ToyUiAddCheckBox` / `AddList` / `AddTextField` | 客户区控件（Gfx 绘制） |
...
| `TOY_UI_EVENT_CLICK` (2) | 客户区点击未命中控件 |
| `TOY_UI_BUTTON_EVENT(id)` (=100+id) | 按钮 `id` 被点击 |
| `TOY_UI_CHECK_EVENT(id)` (=200+id) | 复选框切换 |
| `TOY_UI_LIST_EVENT(id)` (=220+id) | 列表选中（写入输入框缓冲） |
| `TOY_UI_TEXT_EVENT(id)` (=240+id) | 输入框获得焦点 |
| `TOY_UI_KEY_EVENT(hid)` (=300+hid) | 焦点在本用户窗时的按键（USB HID usage；方向键 `0x4F`..`0x52`） |
| `-1` | 无效 wid / 内核错误 |

有焦点的输入框时，可打印键（无 Shift 小写）与退格会写入 `ToyUiGetTextField` 缓冲，**仍返回** KEY 事件。`User/Pkg/Gui` 仍是标签+按钮；课堂演示见 `GUIDEMO.ELF`（标签显示 `key hid=…`）。
...
| `ToyGfxDamageRect(wid, desc)` | 像素矩形 blit（`SYS_DAMAGE_RECT`，单次 ≤64×64） |
| `ToyGfxDrawPixel` / `DrawLine` | 点 / 线（底层仍 DamageRect） |
| `ToyGfxFillRect` / `DrawRect` | 填充矩形 / 矩形框（大矩形自动分块） |

ABI **1.2.0**。颜色 `0x00RRGGBB`。

### 底层 syscall（一般不必直接调）

| 号 | 名 | 用户包装 |
|----|-----|----------|
| 1000 | `SYS_CREATE_WINDOW` | `create_window` / `ToyUiCreateWindow` |
| 1001 | `SYS_DAMAGE` | `damage` / `ToyGfxDamageText` |
| 1002 | `SYS_POLL_INPUT` | `poll_input` / `ToyUiPoll` |
| 1003 | `SYS_UI_BUTTON` | `ui_button` / `ToyUiAddButton` |
| 1100 | `SYS_DAMAGE_RECT` | `ToyGfxDamageRect` / 点线矩形封装 |

权威号表见 `Include/SyscallABI.h`（段内双轨；旧线性号 0/1/16/18… 已废）。
---
...
| `User/Apps/GuiDemo.c` → `GUIDEMO.ELF` | 官方演示 |
| `User/Pkg/Gui/` | 学生模板 |
...
打包：`prepare-rootfs` / `prepare-virt-rootfs.sh` 同步进 Guest。格式见 `Assets/Fonts/README.md`。
