...
| 库 | 内容 |
|----|------|
| **libnet** / **libToyNet** | BSD socket 薄封装；ABI **1.0.0**；`#include <ToyNet.h>` | ✅ **L4**；默认内核 `LWIP=1` |
...
| **libToyNet** | `User/Library/ToyNet/` → `libToyNet.a` | **1.0.0** | ✅ | 默认内核 `LWIP=1`；Guest `lwip on` |
...
| Net：BSD socket | 简化无 `sockaddr`；`send`/`recv`→read/write ✅；**PR-N-dns** errno + Shell `dns`（默认 lwIP） | 完整 `getaddrinfo` / `sockaddr` 后置 |
