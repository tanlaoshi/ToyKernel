...
| **libtoyos** | 稳定 syscall 包装 + 版本宏；`#include <toyos/syscall.h>` / `<toyos/version.h>`；静库 `User/Library/ToyOs/libtoyos.a` | ✅ **L1/L2/A-libc**（CRT **1.3.0**） |
...
| CRT / libtoyos | `TOYOS_CRT_VERSION_*` **1.3.0** | + `lseek` / `fopen` / `realloc` / `strstr`（**A-libc**） |
...
|-----------|-------------|------------|------|------|
| **CRT / libtoyos** | `User/crt/*` → `User/Library/ToyOs/libtoyos.a` | `TOYOS_CRT` **1.3.0** | ✅ | L1/L2 + F4 + **A-libc** |
...
| `<unistd.h>` | `read`/`write`/`close`/`lseek`/`fork`/`wait`/`pipe`/`dup`/`execve`/`brk`/`kill`/`mmap`/`munmap` + 窗口 | 无 `getcwd`/`chdir`/`sleep`/`getpid`；mmap 仅匿名 |
...
| `<toyos/syscall.h>` | SYS 0～29 + `toy_*` 包装 | 改号须同步内核 |
