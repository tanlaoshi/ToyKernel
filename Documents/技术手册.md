...
| 9 | `usb` | `InitializeUsb` | xHCI HID（+ `ps2-kbd` mux）；**EHCI** HID+RMH ✅（ehci-2 TG；MSC→[ehci-3](路线图.md#pr-h-ehci-3)） |
...
| 键盘 / 鼠标 | **Input** | `HalKeyboard*` / `HalMouse*` | `xhci-hid`、`ps2-kbd`（H2）、`ehci`（HID ✅）、`virtio-input` |
