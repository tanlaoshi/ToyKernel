...
| **I** | 分层、扩展边界、目录与启动流程 |
| **II** | Boot / DTB / 板包 / 如何加板 |
| **III** | Driver 框架、如何加驱动、virtio 范例 |
| **IV** | 用户态库、GUI 入门、字体语言、GUI Present |
| **附录** | 与路线图/README/教学内容的分工 |
...
目录与文件清单仍以 [`结构说明.md`](#i-体系结构--目录启动与源文件) 为准；产品排期以 [`路线图.md`](路线图.md) 为准。
...
（见 `KernelModules.c`；失败则停机。教学骨架、识盘≠数据库、与一般 OS 的对照见 [`结构说明.md`](#i-体系结构--目录启动与源文件) 第 2 节。）
...
| `HAL/X64/Hal.c` | 薄封装 |
| `HAL/X64/Page.c` | 页表 Map/GetEntry/Unmap |
| `HAL/X64/Arch.c` | GDT/IDT/LAPIC/中断分发 |
| `HAL/X64/Isr.S` | 中断桩、首次进入用户/内核任务 |
...
| C2 | `HAL/X64/Page.c` | `HalPageMap` / `GetEntry` / `HalLoadPageTable` |
...
板级目录下的 `HAL/**/README.md`、`HAL/X64/NOTES-UEFI-PC.md` 只保留**机型本地**备注，概念与步骤以本手册 II / III 为准。`Documents/` 仅含本手册与 `路线图.md`。
