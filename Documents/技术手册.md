...
> **新人短页**：[`驱动开发指南.md`](驱动开发指南.md)（5 步 + Demo）；拷贝源：[`../HAL/X64/Drivers/_template/`](../HAL/X64/Drivers/_template/)。  
> 匹配：[`驱动匹配规范.md`](驱动匹配规范.md)；设计稿已归档 [`Done/驱动模板设计.md`](Done/驱动模板设计.md)。  
...
| **DTB 兼容串**（后续板包） | 匹配表字段 `TOY_DRIVER.Match` 预留；当前范例多为 `Match = 0`，在 Probe 内扫。规范：[`驱动匹配规范.md`](驱动匹配规范.md) |
