...
| `<unistd.h>` | `read`/`write`/`close`/`fork`/`wait`/`pipe`/`dup`/`execve`/`brk`/`kill`/`mmap`/`munmap` + 窗口 | 无 `lseek`/`getcwd`/`chdir`；mmap 仅匿名 |
...
| `<toyos/syscall.h>` | SYS 0～26 + `toy_*` 包装 | 改号须同步内核 |
...
3. **值得后加的小刀**（非本柱必做）：`realloc`/`strstr`、`lseek`、flags 真生效、libm。
