...
| **libToyUi** | 按钮/标签/复选框/列表/输入框；ABI **1.1.0** | libToyGfx；✅ G15 + **L3** + **A-ui-api** |
...
| libToyUi | `TOY_UI_ABI_VERSION_*` **1.1.0** | `CreateWindow` / `SetLabel` / `AddButton` / `Poll` + 复选框/列表/输入框 |
...
| **libToyUi** | `User/Library/ToyUi/` → `libToyUi.a` | **1.1.0** | ✅ | 窗/按钮 + 复选框/列表/输入框 |
...
| Ui：按钮/标签/事件 | 按钮 + 复选框/列表/输入框 ✅ **1.1.0** | 无键盘入窗；非完整 toolkit |
...
| 库 | 宏 | 当前 |
|----|-----|------|
| libToyUi | `TOY_UI_ABI_VERSION_*` | **1.1.0** |
...
无键盘入窗；输入框内容用 `ToyUiSetTextField` / 点选列表写入。`User/Pkg/Gui` 仍是标签+按钮。
