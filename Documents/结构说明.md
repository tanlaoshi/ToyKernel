...
├── HAL/
│   ├── Board/           # 板包约定 + _template（PR-B0）；真板落在 <Arch>/Board/<board>/
...
│   ├── Arm64/           # Startup、页表、串口、Smp…
│   └── RiscV/           # Startup、页表、串口、Smp…
├── User/                # hello / count / fork / catfile
├── Makefile
└── build.sh
```

**Common 与架构**：上层经 `Include/Hal.h`（含 `HalDevices.h`）访问块设备、USB 输入与网卡；端口 I/O 为 `HalIoRead/Write*`。`PCIe`/`XHCI`/`Net`/`Ata` 驱动头留在 `HAL/<Arch>/`，不进 `Include/`。板级包约定见 [`HAL/Board/README.md`](../HAL/Board/README.md) 与 [`启动与板级支持.md`](启动与板级支持.md)。
