...
> **状态**：分析完成，**待你确认后再做部分 A**（改 `API速查.md`）。  
...
详见 **[`网络API双轨化-执行前分析.md`](网络API双轨化-执行前分析.md)**。

要点：第 1 步只做 `ToyNetConnect`/`ToyNetBind` 改名 + 迁 C 演示；汇编 NETDEMO/NETSRV 可不改；`dirent.c` 已存在；确认后再做第 1 步。
