...
> 概念：[`启动与板级支持.md`](启动与板级支持.md)；约定目录：[`HAL/Board/README.md`](../HAL/Board/README.md)；模板：`HAL/Board/_template/`；排期：**1.3b**（B0～B2 ✅，B3 Duo S）。
...
3. 声明本板设备并 `ToyDrvRegister` / 调用已有 `XxxRegister`
...
```text
cp -a HAL/Board/_template HAL/<Arch>/Board/<board>/
# 编辑 README.md、BoardConfig.h；按需加 Board.c
```

`<Arch>`：`Arm64` 或 `RiscV`（命令行板）。  
`<board>`：如 `milk-v-duo-s`（小写、连字符）。
...
- 块设备 / 网：调用已有 Drv 注册；没有驱动则本柱可只有 UART（B3）
