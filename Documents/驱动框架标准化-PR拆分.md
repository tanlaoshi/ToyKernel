...
> **PR-D-tpl-0** ✅ TG `5e86cc5`。**PR-D-tpl-1** ✅ TG `4109d5c`。**PR-D-tpl-2** 本地齐待 TG；下一刀 **PR-D-tpl-3**。
...
| 4 Demo | **PR-D-tpl-2**     | `DemoDriver.c` + `HalDriverRegister`（`#ifndef` + 注册） | smoke + 串口 `demo: probe/bind`；`TOY_DEMO_DRIVER=0` 无日志；**本地齐待 TG**（NUC 有条件） |
