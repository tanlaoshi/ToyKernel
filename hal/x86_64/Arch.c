...
static void IdtLoad(void) {
    ...
    for (i = 48; i < 256; i++) {
        IdtSet(i, (void *)isr_255);
    }
    IdtSet(VEC_XHCI, (void *)isr_64);
    IdtSet(VEC_TIMER, (void *)isr_65);
    IdtSet(255, (void *)isr_255);
...
void TimerStart(void) {
    ...
    *(volatile UINT32 *)(UINTN)(LAPIC_BASE + LAPIC_TIMER_INIT) = 50000;
    SerialWrite("timer: LAPIC periodic vec=0x41\n");
}

/* 打印异常信息后 cli+hlt 死循环（#PF 时额外打印 CR2） */
static void ExceptionHalt(struct INT_FRAME *F) {
    ConsoleWrite("\nEXCEPTION vec=");
    ConsoleHex32((UINT32)F->Vec);
    ConsoleWrite(" err=");
    ConsoleHex32((UINT32)F->Err);
    ConsoleWrite(" rip=");
    ConsoleHex64(F->Rip);
    if (F->Vec == 14) {
        UINT64 Cr2;
...
UINT64 InterruptDispatch(struct INT_FRAME *F) {
    ...
    if (F->Vec == VEC_TIMER) {
        LapicEoi();
        return SchedOnTimer(F);
    }
    if (F->Vec == VEC_SYSCALL) {
        return SyscallDispatch(F);
    }
    if (F->Vec >= 32 && F->Vec < 48) {
        PicEoi((UINT32)(F->Vec - 32));
        LapicEoi();
        return 0;
    }
    LapicEoi();
...
int ArchInit(void) {
    ...
    ConsoleWrite("IDT ready, LAPIC on, self-IPI ok=");
    ConsoleHex32(gIrqCount);
    ConsoleWrite("\n");
