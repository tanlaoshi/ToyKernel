...
static void CmdHelp(int Argc, char **Argv) {
    ...
    for (int i = 0; i < gCmdCount; i++) {
        ConsoleWrite("  ");
        ConsoleWrite(gCmds[i].Name);
        ConsoleWrite("  ");
        ConsoleWrite(gCmds[i].Help);
        ConsoleWrite("\n");
    }
}
...
void ConsoleRegister(const char *Name, const char *Help,
    ...
    gCmds[gCmdCount].Name = Name;
    gCmds[gCmdCount].Help = Help ? Help : "";
    gCmds[gCmdCount].Handler = Handler;
    gCmdCount++;
}
...
static void RunLine(void) {
    ...
    for (i = 0; i < gCmdCount; i++) {
        if (StrEq(Argv[0], gCmds[i].Name)) {
            gCmds[i].Handler(Argc, Argv);
            return;
        }
    }
...
void ConsoleInit(void) {
    gLen = 0;
    ConsoleRegister("help", "list commands", CmdHelp);
    ConsoleRegister("clear", "clear screen", CmdClear);
    ConsoleRegister("echo", "print arguments", CmdEcho);
    GuiFocusHome();
