...
static UINT8 gVqRxMem[4 * PAGE_SIZE] __attribute__((aligned(4096)));
static UINT8 gVqTxMem[4 * PAGE_SIZE] __attribute__((aligned(4096)));
...
static UINT64 VirtPhys(void *Ptr) {
    return (UINT64)(UINTN)Ptr;
}

static void VqKick(VIRTQ *Q, UINT16 QueueId) {
    if (gIoPort != 0) {
        __asm__ volatile("outw %0, %1" : : "a"(QueueId), "Nd"((UINT16)(gIoPort + 16)));
    }
    if (Q->NotifyBase != 0) {
        volatile UINT16 *Notify = (volatile UINT16 *)(Q->NotifyBase +
                                                        Q->NotifyOff * Q->NotifyMult);
        *Notify = QueueId;
    }
}

static UINTN VqMemSize(UINT16 Size) {
...
static int VqSetup(VIRTQ *Q, UINT16 QueueId, volatile VIRTIO_COMMON_CFG *Common,
    ...
    Common->QueueDesc = VirtPhys(Q->Desc);
    Common->QueueDriver = VirtPhys(Q->Avail);
    Common->QueueDevice = VirtPhys(Q->Used);
    __asm__ volatile("" ::: "memory");
    return 0;
}

static void VqEnable(VIRTQ *Q, UINT16 QueueId) {
...
static UINT16 VqAllocDesc(VIRTQ *Q) {
    UINT16 Idx;
    if (Q->NumFree == 0) {
        return (UINT16)~0;
    }
...
static void VqFreeDesc(VIRTQ *Q, UINT16 Idx) {
    Q->Desc[Idx].Next = Q->FreeHead;
    Q->FreeHead = Idx;
    Q->NumFree++;
}

static void VqSubmitAvail(VIRTQ *Q, UINT16 Head) {
    UINT16 Slot = Q->AvailIdx % Q->Size;
    Q->Avail->Ring[Slot] = Head;
    __asm__ volatile("" ::: "memory");
    Q->AvailIdx++;
    Q->Avail->Idx = Q->AvailIdx;
    VqKick(Q, Q == &gRxQ ? RX_QUEUE_ID : TX_QUEUE_ID);
}

static int VqPopUsed(VIRTQ *Q, UINT16 *Head, UINT32 *Len) {
    UINT16 UsedIdx;
...
static void RxRefillOne(VIRTQ *Q, UINT8 *Buf) {
    UINT16 Head = VqAllocDesc(Q);
    if (Head == (UINT16)~0) {
        return;
    }
    Q->Desc[Head].Addr = VirtPhys(Buf);
    Q->Desc[Head].Len = RX_BUF_SIZE;
    Q->Desc[Head].Flags = VRING_DESC_F_WRITE;
    VqSubmitAvail(Q, Head);
}

static void RxRefillAll(void) {
    int i;
    for (i = 0; i < RX_BUF_COUNT; i++) {
        RxRefillOne(&gRxQ, gRxBufData[i]);
    }
}
...
static int NetSendFrame(const UINT8 *Frame, UINTN FrameLen) {
    ...
    MemSet(gTxBuf, 0, VIRTIO_NET_HDR_LEN);
    MemCpy(gTxBuf + VIRTIO_NET_HDR_LEN, Frame, FrameLen);
    gTxQ.Desc[0].Addr = VirtPhys(gTxBuf);
    gTxQ.Desc[0].Len = (UINT32)(FrameLen + VIRTIO_NET_HDR_LEN);
    gTxQ.Desc[0].Flags = 0;
    gTxQ.Desc[0].Next = 0;
    Slot = gTxQ.AvailIdx % gTxQ.Size;
    gTxQ.Avail->Ring[Slot] = 0;
    __asm__ volatile("" ::: "memory");
    gTxQ.AvailIdx++;
    gTxQ.Avail->Idx = gTxQ.AvailIdx;
    VqKick(&gTxQ, TX_QUEUE_ID);
    return 0;
}
...
static void HandleArp(const ARP_PKT *Arp) {
    ...
    if (Op == ARP_OP_REQUEST && Be32(Arp->TargetIp) == gIp) {
        ...
        Eth->EtherType = Be16(ETH_TYPE_ARP);
        Rep->HwType = Be16(1);
        Rep->ProtoType = Be16(ETH_TYPE_IP);
        Rep->HwLen = 6;
        Rep->ProtoLen = 4;
        Rep->Op = Be16(ARP_OP_REPLY);
        MemCpy(Rep->SenderMac, gMac, 6);
        Rep->SenderIp = Be32(gIp);
...
static void HandleIcmp(const IP_HDR *Ip, const UINT8 *Payload, UINTN PayloadLen) {
    ...
    (void)IpHdrLen;
    Src = Be32(Ip->Src);
    if (gPingWait && Src == gPingTarget && Be16(Icmp->Id) == gPingId) {
        gPingWait = 0;
    }
}
...
static void HandleIpPacket(const UINT8 *Pkt, UINTN Len) {
    ...
    if (Be16(Eth->EtherType) == ETH_TYPE_ARP) {
        if (Len >= ETH_HDR_LEN + sizeof(ARP_PKT)) {
            HandleArp((const ARP_PKT *)(Pkt + ETH_HDR_LEN));
        }
        return;
    }
    if (Be16(Eth->EtherType) != ETH_TYPE_IP || Len < ETH_HDR_LEN + IP_HDR_LEN) {
        return;
    }
...
static void NetProcessRx(void) {
    ...
    while (VqPopUsed(&gRxQ, &Head, &Len)) {
        Buf = (UINT8 *)(UINTN)gRxQ.Desc[Head].Addr;
        if (Len > VIRTIO_NET_HDR_LEN) {
            HandleIpPacket(Buf + VIRTIO_NET_HDR_LEN, Len - VIRTIO_NET_HDR_LEN);
        }
        VqFreeDesc(&gRxQ, Head);
        RxRefillOne(&gRxQ, Buf);
    }
}
...
static int VirtioParseCaps(UINT8 Bus, UINT8 Dev, UINT8 Fn,
    ...
    while (CapPtr >= 0x40) {
        ...
        if (CapId == PCI_CAP_VENDOR && CapLen >= 16) {
            ...
            if ((BarLo & 1) == 0 && Base != 0) {
                VmmMapRange(Base, Base, 0x100000, PTE_PRESENT | PTE_WRITABLE);
            }
...
static int VirtioNetStart(UINT8 Bus, UINT8 Dev, UINT8 Fn, UINT64 BarPhys) {
    ...
    VmmMapRange(BarPhys, BarPhys, 0x10000, PTE_PRESENT | PTE_WRITABLE);
    (void)BarPhys;
    ...
    if (VqSetup(&gRxQ, RX_QUEUE_ID, Common, NotifyBase, NotifyMult) != 0) {
        return -1;
    }
    if (VqSetup(&gTxQ, TX_QUEUE_ID, Common, NotifyBase, NotifyMult) != 0) {
        return -1;
    }
    Common->DeviceStatus = VIRTIO_STATUS_ACK | VIRTIO_STATUS_DRIVER |
                           VIRTIO_STATUS_FEATURES_OK | VIRTIO_STATUS_DRIVER_OK;
    __asm__ volatile("" ::: "memory");
    VqEnable(&gRxQ, RX_QUEUE_ID);
    VqEnable(&gTxQ, TX_QUEUE_ID);
...
int NetInit(void) {
    ...
    if (!VirtioFindNet(&Bus, &Dev, &Fn, &Bar)) {
        SerialWrite("net: virtio-net not found\n");
        return 0;
    }
    if (VirtioNetStart(Bus, Dev, Fn, Bar) != 0) {
        SerialWrite("net: virtio init failed\n");
        return 0;
    }
    gNetOk = 1;
    SerialWrite("net: virtio-net up\n");
    return 0;
}
...
void NetInfo(void) {
    char IpBuf[20];
    if (!gNetOk) {
        SerialWrite("net: not available\n");
        return;
    }
    NetFormatIp(gIp, IpBuf, sizeof(IpBuf));
    SerialWrite("net: mac ");
    for (int i = 0; i < 6; i++) {
        SerialWrite(Uint8ToDecimal(gMac[i]));
        if (i < 5) {
            SerialWrite(":");
        }
    }
...
int NetPing(const char *Host, int TimeoutMs) {
    ...
    MemCpy(Eth->Src, gMac, 6);
    Eth->EtherType = Be16(ETH_TYPE_IP);
    ...
    Ip->TotalLen = Be16(IpTotal);
    Ip->Id = Be16(0x1234);
    Ip->Checksum = 0;
    Ip->Checksum = Sum16((UINT8 *)Ip, IP_HDR_LEN);
    Icmp = (ICMP_HDR *)(Frame + ETH_HDR_LEN + IP_HDR_LEN);
    gPingId = 0x4F53;
    gPingSeq++;
    Icmp->Type = ICMP_ECHO_REQUEST;
    Icmp->Code = 0;
    Icmp->Id = Be16(gPingId);
    Icmp->Seq = Be16(gPingSeq);
    Payload = Frame + ETH_HDR_LEN + IP_HDR_LEN + ICMP_HDR_LEN;
    for (int i = 0; i < PING_PAYLOAD; i++) {
        Payload[i] = (UINT8)i;
    }
    Icmp->Checksum = 0;
    Icmp->Checksum = Sum16((UINT8 *)Icmp, ICMP_HDR_LEN + PING_PAYLOAD);
    gPingTarget = Target;
    gPingWait = 1;
