...
int PciScanUSBControllers(USB_CONTROLLER *Controllers, int MaxControllers) {
        ...
        for (int Device = 0; Device < 32; Device++) {
            for (int Function = 0; Function < 8; Function++) {
                ...
                if (Class == 0x0C && Subclass == 0x03) {
                    ...
                    ConsoleWrite("USB: ");
                    ConsoleWrite(TypeStr);
                    ConsoleWrite(" at ");
                    ConsoleWrite(Uint64ToHex(BaseAddress));
                    ConsoleWrite("\n");
    ...
    if (!Found) {
        ConsoleWrite("No USB Controller found!\n");
    } else {
        ConsoleWrite("PCI Scan Complete\n");
    }
...
int PciEnableMsi(USB_CONTROLLER *Dev, UINT8 Vector) {
    UINT32 Cmd = PciReadConfig(Dev->Bus, Dev->Device, Dev->Function, 0x04);
    Cmd |= (1u << 10);
    PciWriteConfig(Dev->Bus, Dev->Device, Dev->Function, 0x04, Cmd);

    int Cap = PciFindCap(Dev->Bus, Dev->Device, Dev->Function, 0x11);
    if (Cap) {
        UINT32 TableDw = PciReadConfig(Dev->Bus, Dev->Device, Dev->Function, (UINT8)(Cap + 4));
        UINT32 Bir = TableDw & 7;
        UINT32 Off = TableDw & 0xFFFFFFF8u;
        if (Bir > 5 || Dev->Bar[Bir] == 0) {
            ConsoleWrite("MSI-X: bad BIR\n");
            return 0;
        }
        UINT64 Entry = Dev->Bar[Bir] + Off;
        ...
        *(volatile UINT32 *)(UINTN)(Entry + 12) = 0;
        ...
        UINT32 Dw0 = PciReadConfig(Dev->Bus, Dev->Device, Dev->Function, (UINT8)Cap);
        Dw0 |= (1u << 31);
        Dw0 &= ~(1u << 30);
        PciWriteConfig(Dev->Bus, Dev->Device, Dev->Function, (UINT8)Cap, Dw0);
        ConsoleWrite("MSI-X enabled vec=");
        ConsoleHex32(Vector);
        ConsoleWrite("\n");
        return 1;
    }

    Cap = PciFindCap(Dev->Bus, Dev->Device, Dev->Function, 0x05);
    if (!Cap) {
        ConsoleWrite("PCI: no MSI/MSI-X\n");
        return 0;
    }
    UINT32 Dw0 = PciReadConfig(Dev->Bus, Dev->Device, Dev->Function, (UINT8)Cap);
    UINT16 Ctl = (UINT16)(Dw0 >> 16);
    if (Ctl & (1u << 7)) {
        PciWriteConfig(Dev->Bus, Dev->Device, Dev->Function, (UINT8)(Cap + 4), 0xFEE00000u);
        PciWriteConfig(Dev->Bus, Dev->Device, Dev->Function, (UINT8)(Cap + 8), 0);
        UINT32 DataDw = PciReadConfig(Dev->Bus, Dev->Device, Dev->Function, (UINT8)(Cap + 12));
        PciWriteConfig(Dev->Bus, Dev->Device, Dev->Function, (UINT8)(Cap + 12),
                       (DataDw & 0xFFFF0000u) | Vector);
    } else {
        PciWriteConfig(Dev->Bus, Dev->Device, Dev->Function, (UINT8)(Cap + 4), 0xFEE00000u);
        UINT32 DataDw = PciReadConfig(Dev->Bus, Dev->Device, Dev->Function, (UINT8)(Cap + 8));
        PciWriteConfig(Dev->Bus, Dev->Device, Dev->Function, (UINT8)(Cap + 8),
                       (DataDw & 0xFFFF0000u) | Vector);
    }
    Ctl |= 1;
    Dw0 = PciReadConfig(Dev->Bus, Dev->Device, Dev->Function, (UINT8)Cap);
    PciWriteConfig(Dev->Bus, Dev->Device, Dev->Function, (UINT8)Cap,
                   (Dw0 & 0xFFFFu) | ((UINT32)Ctl << 16));
    ConsoleWrite("MSI enabled vec=");
    ConsoleHex32(Vector);
    ConsoleWrite("\n");
    return 1;
}