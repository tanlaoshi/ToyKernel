 ...
 * Pmm.c — 物理页分配器（位图实现）
 *
 * 遍历 UEFI Conventional 内存建立空闲位图，再扣除内核/配置/帧缓冲等保留区。
 * 最大跟踪 512MB（PMM_MAX_PAGES 页），位图存放在内核 .bss。
 */
#include "Pmm.h"
#include "Console.h"
...
#define PMM_MAX_PAGES (128u * 1024u)
...
static UINT8  gBitmap[PMM_MAX_PAGES / 8];
...
int PmmInit(const PMM_BOOT_INFO *Info) {
    ...
    for (UINT32 i = 0; i < (PMM_MAX_PAGES / 8); i++) {
        gBitmap[i] = 0xFF;
    }
    gFreePages = 0;
    ...
    ConsoleWrite("PMM: ");
    ConsoleHex64(gFreePages << PAGE_SHIFT);
    ConsoleWrite(" bytes free / ");
    ConsoleHex64((UINT64)gMaxPage << PAGE_SHIFT);
    ConsoleWrite(" bytes tracked (");
    ConsoleHex32((UINT32)gFreePages);
    ConsoleWrite(" pages free)\n");
    return 0;
}
