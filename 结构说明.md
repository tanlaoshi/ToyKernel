...
```
ToyKernel/
├── Common/          # 上层逻辑（经 hal.h 访问硬件）
├── Hal/
│   ├── hal.h
│   ├── x86_64/      # Arch.c、Isr.S、Hal.c、Page.c、link.ld
│   ├── riscv/       # 占位
│   └── arm64/       # 占位
...
**Common 与架构**：上层经 `hal.h` 访问硬件。端口 I/O 已下沉为 `HalIoRead/Write*`（`Hal/*/Io.c`）；`Serial`/`Ata`/`PCIe`/`Net`（virtio legacy notify）不再内嵌 `in`/`out` 汇编。寄存器地址（如 COM1 `0x3F8`、ATA `0x1F0`、PCI `0xCF8`）仍在驱动里，属设备约定而非 CPU 指令。

**VMM 分层**：

```
Common/VirtualMemory.c   地址空间、Clone、CopyFrom/ToUser
        ↓ HalPage*
...
| 文件 | 说明 |
|------|------|
| `Common/Kernel.c` | `KernelEntry`、模块表、Shell/Gui/Worker、`exec`/`ps`/网络命令 |
| `Common/BootConfig.h` | 与 Boot 共享结构 |
...
| 顺序 | 文件 / 位置 | 做什么 / 看什么 |
|------|-------------|-----------------|
| A1 | `Common/BootConfig.h` | 与 Boot 共享的字段；改布局必须同步 `ToyBoot/Boot.c` |
| A2 | `ToyBoot/Boot.c` | GOP、读 `Kernel.elf`（同卷/跨卷）、`ExitBootServices`、跳转 |
| A3 | `Hal/x86_64/link.ld` | 为何必须加载到 `0x100000` |
| A4 | `Common/Kernel.c` 的 `KernelEntry` | 最早 `VideoSet`、模块表、创建任务 |
| A5 | `Common/Module.c` | 顺序 Init 框架 |
