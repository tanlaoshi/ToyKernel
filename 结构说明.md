...
**x86 与 Arm/RiscV 的 Boot 不同**；Common 从 `BOOT_INFO` 起合流。分层硬规则见 [`架构分层.md`](架构分层.md) 第 3 节。
...
改 UEFI 侧 `BOOT_CONFIG` 布局时，同步 `ToyBoot/Boot.c` 与 `HAL/X86_64/BootConfig.h`。Arm/RiscV 无 `BOOT_CONFIG`，由该 Arch Startup 直接填 `BOOT_INFO`。Common **只**依赖 `BOOT_INFO`。
