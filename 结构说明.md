...
**分层与对外接口**见 [`架构分层.md`](架构分层.md)（PR-L0）。  
路线图见 [`路线图.md`](路线图.md)。
...
**x86 与 Arm/RiscV 的 Boot 不同**；Common 从 `BOOT_INFO` 起合流。分层硬规则见 [`架构分层.md`](架构分层.md) 第 3 节。Arm64 / RiscV **不走 UEFI**、不移植 `BOOTAA64.EFI`。
...
**Arm64 / RiscV（自有 Boot，非 UEFI）**

```
QEMU virt（Arm：DTB ± 小型非 UEFI loader；RiscV：OpenSBI ± 小型非 UEFI loader）
...
改 UEFI 侧 `BOOT_CONFIG` 布局时，同步 `ToyBoot/Boot.c` 与 `HAL/X86_64/BootConfig.h`。Arm/RiscV **无** `BOOT_CONFIG`/UEFI，由该 Arch Startup 直接填 `BOOT_INFO`。Common **只**依赖 `BOOT_INFO`。
...
### 2.1 当前模块初始化顺序（`KernelModules.c` → `gModules[]`）

| 顺序 | 模块 | Init | 职责 |
|------|------|------|------|
| 1 | serial | InitSerial | COM1，最早日志 |
| 2 | mem | InitMem | PMM |
| 3 | vmm | InitVmm | 页表 + 映射 FB/MMIO + 开分页 |
| 4 | video | InitVideo | 帧缓冲、默认主题色清屏（不读盘） |
| 5 | cpu | InitCpu | GDT/IDT/LAPIC + syscall 门 |
| 6 | smp | InitSmp | 拉起 AP |
| 7 | fs | InitFileSystemModule | Block→GPT→FAT；识盘（`TOYOS.ID`）并挂卷 |
| 8 | usb | InitUsb | XHCI 键盘/鼠标 |
| 9 | net | InitNetModule | virtio-net + UDP/TCP |
| 10 | gui | InitGuiModule | **DbInit → ThemeLoad（DB/`THEME.CFG`）** 后桌面与窗口 |
| 11 | sched | InitSched | 调度器表 |
| 12 | console | InitConsole | Shell 内置命令 |

任一 Init 非 0 → 停机。`usb`/`net` 插在挂根之后、读主题之前，属于「其余驱动」提前插入，不改变「Video 先于库、库晚于 FS」这条主线。
...
| 5 | CPU / 中断 | `cpu`（+ `smp`） |
| 6 | 块设备 | `fs` 内 ATA / Block |
| 7 | 分区表 + FS 探测 | `fs` 内 GPT + FAT |
| 8 | 识别系统卷 | 卷上有 `TOYOS.ID` |
| 9 | 挂载根 | `FileSystem` 默认卷 `TOYOS:` |
| 10 | 数据库 / 配置 | `gui`：`DbInit` → `ThemeLoad`（`TOYOS.DB`，可导入 `THEME.CFG`） |
| 11 | 其余驱动 | `usb`、`net`（ToyOS 插在 9 与 10 之间） |
| 12 | GUI / Shell / 调度 | `gui` → `sched` → `console` |
...
| `Fat.c/h` | FAT16/32 目录读/写（8.3+LFN，`.`/`..`，`mkdir`/`rmdir`，写 ≤1MB） |
...
- FAT：8.3 + LFN；`.`/`..`；`mkdir`/`rmdir`；写 ≤1MB；统一 `FAT_*` 错误码
