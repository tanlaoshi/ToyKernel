...
**Common 与架构的关系**：设计目标是协议/策略层与 CPU 无关；实际上仍有 **x86 端口 I/O** 留在 Common（如 `Serial.c` COM1、`Ata.c`、`PCIe.h` 的 `inl`/`outl`、`Net.c` 的 legacy notify）。中断/GDT/页表在 `Hal/x86_64/`。后续多架构时应把 I/O 下沉到 Hal。

**VMM 分层**（已完成）：

```
Common/VirtualMemory.c          地址空间策略、用户区布局、VirtualMemoryCopyFromUser
...
内核低地址区（0–512MB）为**恒等映射**（2MB 大页）；高位 MMIO（帧缓冲、LAPIC、XHCI）在启用分页前额外映射。用户态页映射见 `VirtualMemory.c` / `Syscall.c`。
...
KernelEntry()
  ...
  └─ SchedulerStart() → LAPIC 定时器 → 抢占式多任务
```
...
| 6 | fs | InitFileSystemModule | ATA → GPT → FAT → ls/cat |
...
```
硬件中断
  → Isr.S: isr_N → IsrCommon（保存寄存器）
  → Arch.c: InterruptDispatch()
       ├─ vec < 32  → ExceptionHalt（打印后死循环）
       ├─ VEC_XHCI (0x40) → XhciIrq()
       ├─ VEC_TIMER (0x41) → SchedulerOnTimer()，可能切换 RSP
       ├─ VEC_SYSCALL (0x80) → SyscallDispatch()
       └─ PIC 向量 → PicEoi
  → IsrCommon: 若 rax≠0 则 mov rsp,rax（上下文切换）
  → iretq 返回任务
```
...
| 文件 | 说明 |
|------|------|
| `Scheduler.c` / `Scheduler.h` | 最多 8 任务、时间片轮转、抢占切换 |
| `PhysicalMemory.c` / `PhysicalMemory.h` | 基于 UEFI 内存映射的位图物理页分配器 |
| `Common/VirtualMemory.c` / `VirtualMemory.h` | **策略层**：地址空间、`VirtualMemoryCopyFromUser`；映射委托 `HalPage*` |
| `Common/Elf.c` / `Elf.h` | 静态 ELF64 加载 |
| `Process.c` / `Process.h` | `ProcessExec`、`ProcessRunDemo` |
| `Syscall.c` / `Syscall.h` | int 0x80；`SYS_write` 经 `VirtualMemoryCopyFromUser` 读用户缓冲 |
...
| `FileSystem.c` / `FileSystem.h` | 组装 ATA+GPT+FAT，注册 `ls`/`cat` 命令 |

### 4.8 网络

| 文件 | 说明 |
|------|------|
| `Net.c` / `Net.h` | virtio-net、ARP、ICMP、`NetSendIp` |
| `Udp.c` / `Udp.h` | UDP bind/send/接收队列 |
| `Tcp.c` / `Tcp.h` | 单连接 TCP listen 回显 / connect |
...
| ls / cat | FileSystem.c | 根目录列表 / 读文件 |
| info / ps / mem / memtest / runuser / exec | Kernel.c | 视频信息 / 任务列表 / 内存 / runuser / FAT 加载 ELF |
| net / ping / udplisten / udpsend / tcplisten / tcpstatus | Kernel.c | 网络调试 |
---
...
```
┌─────────────────────────────────────────────────────────────┐
│ L4 应用编排    Kernel.c  FileSystem.c  Module.c  Syscall.c          │
├─────────────────────────────────────────────────────────────┤
│ L3 子系统      Arch  XHCI  Scheduler  VirtualMemory  Console  Fat  Gpt    │
├─────────────────────────────────────────────────────────────┤
│ L2 驱动/库     PCIe  Ata  Video  UI  PhysicalMemory  HIDKeyboard       │
├─────────────────────────────────────────────────────────────┤
...
PhysicalMemory.h         ←── PhysicalMemory.c, VirtualMemory.c, Syscall.c, Kernel.c
VirtualMemory.h         ←── VirtualMemory.c, Syscall.c, Kernel.c

Arch.h        ←── Arch.c, Scheduler.c, XHCI.c, Syscall.c, Kernel.c
Isr.S         ←──（汇编，链接期依赖 Arch.c 的 InterruptDispatch）

Scheduler.h       ←── Scheduler.c, Arch.c, Kernel.c
...
Ata.h         ←── Ata.c, Gpt.c, Fat.c, FileSystem.c
Gpt.h         ←── Gpt.c, FileSystem.c
Fat.h         ←── Fat.c, FileSystem.c
FileSystem.h          ←── FileSystem.c, Kernel.c
...
serial ──► mem ──► vmm ──► video ──► cpu ──► fs ──► usb ──► sched ──► console
  ...
  │         │       │        └─ 需要帧缓冲 MMIO 已映射（InitVmm 中 VirtualMemoryMapIdentity）
...
| Scheduler | 无硬件 | Console（日志） |
| Console | Video + Serial | 前面各模块注册的命令 |

**注意**：`Module.c` 在打印 `[mod]` 日志时调用 `ConsoleWrite`，但 `ConsoleInit()` 在**最后**才执行——因此启动日志实际走 `ConsoleWrite` → `SerialWrite` + `VideoDrawString`，此时命令表尚未建立，属正常行为。
...
```
           ...
           异常(<32)   VEC_XHCI   VEC_TIMER    VEC_SYSCALL
              │           │          │              │
              ▼           ▼          ▼              ▼
        ExceptionHalt  XhciIrq  SchedulerOnTimer  SyscallDispatch
                           │          │              │
                           │          ▼              ├─► Serial + Video (write)
                           │     Scheduler.c            └─► Console (exit 消息)
                           │     切换任务 RSP
                           ▼
                    XHCI 键盘队列
                           │
...
**耦合特征**：`Arch.c` 在编译期直接 `#include` 了 `XHCI.h`、`Scheduler.h`、`Syscall.h`，是**中断层的集中分发器**，新增 IRQ 源需改 `Arch.c`（或通过函数指针解耦，当前未做）。
...
`Console` 是 **I/O 汇聚点**：所有命令处理函数（Kernel.c、FileSystem.c）通过 `ConsoleWrite` 输出；输入来自 Shell 任务（USB + 串口双通道）。
...
BootConfig.MemoryMap
        │
        ▼
     PhysicalMemory.c ──alloc──► VirtualMemory.c（页表页）
        │                  │
        │                  └──► Syscall.c（用户页映射）
        │
        └──► Kernel.c（memtest 命令）
...
```
Ata.c ──PIO 扇区读──► Gpt.c ──分区 LBA──► Fat.c ──目录/文件──► FileSystem.c ──注册──► Console
```
...
```
Kernel.c: SchedulerCreate / SchedulerStart
              │
              ▼
         Scheduler.c ◄──── TimerStart() ──── Arch.c
              │
              ▼
...
```
runuser 命令 (Kernel.c)
    → ProcessRunDemo (Syscall.c)
        → PhysicalMemoryAllocatePage + VirtualMemoryMapPage
...
| Syscall 直接读用户指针 | `VirtualMemoryCopyFromUser` 校验用户区 PTE | 可扩展 `copy_to_user` |
...
- `SYS_write` 使用 `VirtualMemoryCopyFromUser` 校验用户指针
...
| Isr.S | IsrCommon, SchedEnter, Isr0..Isr65 |
| Scheduler.c | SchedulerInit, SchedulerCreate, SchedulerOnTimer, SchedulerStart |
| PhysicalMemory.c | PhysicalMemoryInit, PhysicalMemoryAllocatePage(s), PhysicalMemoryFreePage(s) |
| Console.c | ConsoleInit, ConsoleRegister, ConsoleOn* |
| Serial.c | SerialInit, SerialWrite, SerialHexFormat |
| Video.c | VideoSet, VideoDrawPixel, VideoDrawChar, VideoDrawString |
| UI.c | UiDrawLine, UiDrawRectangle, UiDrawButton, ... |
| PCIe.c | PciRead/WriteConfig, PciScanUSBControllers, PciEnableMsi |
| XHCI.c | XhciInit, XhciIrq, XhciDequeueKeyboard |
