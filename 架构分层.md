...
│             x86：ToyBoot/UEFI；Arm/RiscV：非 UEFI 自有 Boot   │
└─────────────────────────────────────────────────────────────┘
```

| 层 | 目录 / 仓库 | 职责一句话 |
|----|-------------|------------|
| **Boot** | 各 `HAL/<Arch>/Startup`；x86 另有 `ToyBoot/` | 填 `BOOT_INFO` 交给 Common。**x86** = UEFI/ToyBoot；**Arm/RiscV** = 自有 Boot。**禁止**把 UEFI 当跨架构必经 |
...
**硬规则**：HAL 收纳 **全部** 架构相关代码。Common 不关心 GDT 选择子、`EM_X86_64`、UEFI、PL011 vs COM1、GIC vs LAPIC。Arm64 / RiscV **与 UEFI 无关**（自有 Boot，不是 ToyBoot-as-UEFI 移植）。
...
| ❌ Arm64 / RiscV **不得**走 UEFI / ToyBoot | 自有 Boot → 本 Arch `Startup`；禁止 `BOOTAA64.EFI` / AAVMF / RiscVVirt EDK2 当必经 |
...
Boot **不是**「一套 UEFI 走天下」。各 Arch 自有 Boot，在 `HAL/<Arch>/Startup` 填好 `BOOT_INFO` 后，Common 才开始。**禁止**把 UEFI 当跨架构必经。

| Arch | Boot | 谁填 `BOOT_INFO` |
|------|------|------------------|
| x86_64 | UEFI + ToyBoot（OVMF）→ `BOOT_CONFIG` | `HAL/X86_64/Startup` |
| Arm64 | 自有 Boot：QEMU `virt` + DTB ± 小型非 UEFI loader（`-kernel` 已是正确入口） | `HAL/Arm64/Startup` |
| RiscV | 自有 Boot：OpenSBI（QEMU 默认）± 小型非 UEFI loader | `HAL/RiscV/Startup` |

相同用户界面 = **Common Gui**。亮屏是该 Arch 的 **HAL Video**（virt 上 ramfb/virtio-gpu，不是 GOP/UEFI）+ 自有 Boot，**不是**克隆 x86 UEFI 路径。块设备：virtio-blk 在 HAL 注册，再接 Common FAT。
...
- **不做**：移植 ToyBoot 为 `BOOTAA64.EFI`、AAVMF、RiscVVirt EDK2。
...
| 系统调用 | `Syscall.h` | `SyscallInit`、`SyscallDispatch`；号 0–15（含 `execve`/`pipe`/`dup`）见下 |
...
| 资产 | 说明 |
|------|------|
| `User/*.S` / `User/*.c` + `User/crt/` | Ring 3；CRT1：`printf`/`strlen`/`memcpy`/`malloc` + `toy_syscall` |
...
各 Arch 自有 Boot / Startup 填 BOOT_INFO
  （x86：ToyBoot GOP → BOOT_CONFIG → Startup；
    Arm/RiscV：DTB/OpenSBI 等 → 本 Arch Startup，非 UEFI）
