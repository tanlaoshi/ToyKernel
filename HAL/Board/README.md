...
- **Makefile `BOARD=`**：骨架在 **B2**；本 PR 只定包形状，不改构建。
...
| `Board.c` / `Board.h` | B2/B3 起 | Startup 调用的填充辅助；兼容 ID 表；**不**进 Common |
...
```bash
cp -a HAL/Board/_template HAL/Arm64/Board/milk-v-duo-s   # 例；B2 起再进 Makefile
